...
const uploadDir = path.join(__dirname, '../public/Uploads/outward');
...
async function watermarkPDF(inputPath, outwardNumber, outputPath) {
  try {
    console.log('Watermarking PDF:', { inputPath, outwardNumber, outputPath });
...
app.post('/api/outward', upload.single('document'), async (req, res) => {
  const {
  ...
  let documentPath = req.file ? `/Uploads/outward/${req.file.filename}` : null;
  ...
  generateOutwardNumber(division, section, procedures || 'UNKNOWN', topic || 'UNKNOWN', subtopic || 'UNKNOWN', async (err, outwardNumber) => {
    if (err) {
    ...
    if (req.file) {
      ...
      const newFilename = outwardNumber.replace(/\//g, '-') + '.pdf';
      const newPath = path.join(uploadDir, newFilename);
      try {
        await fs.access(tempPath);
        console.log(`Temporary file exists: ${tempPath}`);
        await fs.rename(tempPath, newPath);
        documentPath = `/Uploads/outward/${newFilename}`;
        console.log(`File renamed from ${req.file.filename} to ${newFilename}`);
      } catch (renameError) {
        console.error('Error renaming file:', renameError);
        return res.status(500).json({ message: `Failed to rename file: ${renameError.message}` });
      }
    }
    ...
    pool.query(insertQuery, insertValues, async (err, result) => {
      ...
      if (documentPath) {
        ...
        try {
          ...
          console.error('Failed to watermark PDF:', watermarkErr);
        }
      }
...
app.get('/api/senders', (req, res) => {
  pool.query(
    ...
    (err, results) => {
      ...
      res.json(results.map(r => r.sender));
    }
  );
});
...
app.get('/api/report/sender-receiver-region-stats', (req, res) => {
  const { startDate, endDate, sender } = req.query;
  if (!startDate || !endDate || !sender) {
    return res.status(400).json({ message: 'Start date, end date, and sender are required' });
  }
  ...
  pool.query(
    `SELECT addressedTo, region, COUNT(*) as count
     FROM outward_documents
     WHERE date_sent BETWEEN ? AND ? AND sender = ? AND region IN ('A', 'B', 'C', 'D')
     GROUP BY addressedTo, region
     ORDER BY addressedTo, region`,
    [startDate, endDate, sender],
    (err, results) => {
      if (err) {
        console.error('Error fetching sender receiver-region stats:', err);
        return res.status(500).json({ message: 'Database error' });
      }
      res.json(results);
    }
